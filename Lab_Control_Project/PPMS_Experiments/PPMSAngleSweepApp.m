classdef PPMSAngleSweepApp < handle
    % PPMSAngleSweepApp - Multiplexed R vs. Angle (Rotator Sweep) at Static Field
    ...
    properties (Access = private)
        ...
        StaticFieldEdit, StaticTempEdit
        StartAngleEdit, EndAngleEdit, SpeedEdit, IntervalEdit
    ...
    methods
        function app = PPMSAngleSweepApp()
            % Construct the GUI
            app.UIFigure = uifigure('Name', 'PPMS Angular Dependence Sweep', 'Position', [100, 50, 1150, 880]);
            ...
            % 8 Rows to fit the Static Environment Panel
            controlLayout = uigridlayout(controlPanel, [8, 1], 'RowHeight', {'fit', 'fit', 'fit', 'fit', '1x', 'fit', 'fit', 50});
            
            app.createConnectionPanel(controlLayout);
            app.createStaticEnvPanel(controlLayout);
            ...
            title(app.PlotAxes, 'Resistance vs. Rotator Angle');
        ...
        function createStaticEnvPanel(app, parent)
            p = uipanel(parent, 'Title', 'Static Environment');
            g = uigridlayout(p, [2, 2], 'ColumnWidth', {'1x', 100});
            
            uilabel(g, 'Text', 'Target Temp (K):');  app.StaticTempEdit = uieditfield(g, 'numeric', 'Value', 300.0);
            uilabel(g, 'Text', 'Target Field (Oe):'); app.StaticFieldEdit = uieditfield(g, 'numeric', 'Value', 1000.0);
        end
        
        function createSweepPanel(app, parent)
            p = uipanel(parent, 'Title', 'Rotator Angle Sweep');
            g = uigridlayout(p, [4, 2], 'ColumnWidth', {'1x', 100});
            
            uilabel(g, 'Text', 'Start Angle (deg):'); app.StartAngleEdit = uieditfield(g, 'numeric', 'Value', 0.0);
            uilabel(g, 'Text', 'End Angle (deg):');   app.EndAngleEdit = uieditfield(g, 'numeric', 'Value', 360.0);
            uilabel(g, 'Text', 'Speed (deg/sec):');   app.SpeedEdit = uieditfield(g, 'numeric', 'Value', 1.0);
        ...
        function saveConfig(app)
            ...
            config.StaticTemp    = app.StaticTempEdit.Value;
            config.StaticField   = app.StaticFieldEdit.Value;
            config.StartAngle    = app.StartAngleEdit.Value;
            config.EndAngle      = app.EndAngleEdit.Value;
            config.Speed         = app.SpeedEdit.Value;
        ...
        function loadConfig(app)
            ...
            try
                ...
                app.StaticTempEdit.Value  = c.StaticTemp;
                app.StaticFieldEdit.Value = c.StaticField;
                app.StartAngleEdit.Value  = c.StartAngle;
                app.EndAngleEdit.Value    = c.EndAngle;
                app.SpeedEdit.Value       = c.Speed;
        ...
        function runExperiment(app)
            ...
            fprintf(fileID, '%% Static Env: Temp = %f K | Field = %f Oe\n', app.StaticTempEdit.Value, app.StaticFieldEdit.Value);
            fprintf(fileID, '%% Rotator Sweep: %f deg to %f deg at %f deg/sec\n', app.StartAngleEdit.Value, app.EndAngleEdit.Value, app.SpeedEdit.Value);
            ...
            try
                ...
                % 1. PPMS: Go to Static Temperature
                app.PlotAxes.Title.String = sprintf('Ramping to Static Temp (%.1f K)...', app.StaticTempEdit.Value);
                app.PPMS.setTemperature(app.StaticTempEdit.Value, 10.0, 'FastSettle');
                while app.IsRunning
                    [currentTemp, status] = app.PPMS.getCurrentTemperature();
                    if strcmpi(status, 'Stable') && abs(currentTemp - app.StaticTempEdit.Value) < 0.2
                        break;
                    end
                    pause(1);
                end
                ...
                % 2. PPMS: Go to Static Magnetic Field
                app.PlotAxes.Title.String = sprintf('Ramping to Static Field (%.1f Oe)...', app.StaticFieldEdit.Value);
                app.PPMS.setMagneticField(app.StaticFieldEdit.Value, 100.0, 'Linear', 'Driven');
                while app.IsRunning
                    [currentField, ~] = app.PPMS.getCurrentField();
                    tolerance = max(2.0, 0.01 * abs(app.StaticFieldEdit.Value));
                    if abs(currentField - app.StaticFieldEdit.Value) < tolerance
                        break;
                    end
                    pause(1);
                end
                ...
                % 3. PPMS: Move Rotator to Start Angle
                app.PlotAxes.Title.String = sprintf('Moving Rotator to Start Angle (%.1f deg)...', app.StartAngleEdit.Value);
                app.PPMS.setRotatorAngle(app.StartAngleEdit.Value, app.SpeedEdit.Value);
                while app.IsRunning
                    [currentAngle, ~] = app.PPMS.getRotatorAngle();
                    if abs(currentAngle - app.StartAngleEdit.Value) < 0.5
                        break;
                    end
                    pause(1);
                end
                ...
                % 4. PPMS: Begin the Angle Sweep
                app.PlotAxes.Title.String = 'Resistance vs. Rotator Angle Sweep';
                app.PPMS.setRotatorAngle(app.EndAngleEdit.Value, app.SpeedEdit.Value);
                pause(2); 
                
                % 5. Main R vs Angle Multiplexed Measurement Loop
                while app.IsRunning
                    loopTimer = tic;
                    ...
                    for c = 1:numChannels
                        ...
                        stepVoltages(c) = rVal;
                        if abs(rVal) < 1e6 
                            addpoints(app.DataLines{c}, currentAngle, rVal);
                        end
                        drawnow limitrate;
                    end
                    ...
                    isSweepingUp = app.EndAngleEdit.Value > app.StartAngleEdit.Value;
                    
                    if isSweepingUp && (currentAngle >= app.EndAngleEdit.Value - 0.5)
                        break;
                    elseif ~isSweepingUp && (currentAngle <= app.EndAngleEdit.Value + 0.5)
                        break;
                    end
                ...
                uialert(app.UIFigure, 'Angle sweep finished successfully!', 'Complete');
